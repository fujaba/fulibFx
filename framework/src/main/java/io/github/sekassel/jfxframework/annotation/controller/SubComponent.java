 ...
 * <p>
 * Component fields annotated with @SubController will be initialized and rendered with the controller they are defined in.
 * The instance still needs to be provided by the user oder using dependency injection.
 * The instance will be used if the fxml specifies a sub-controller with the same type.
 * <p>
 * Provider fields annotated with @SubController can be used if the fxml specifies a sub-controller with the same type.
